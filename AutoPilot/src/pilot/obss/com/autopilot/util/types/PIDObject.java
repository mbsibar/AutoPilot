...
public class PIDObject extends AlgorithmObject{
	...
	public PIDObject(String name, float p, float i, float d, int maxListSize, float minOutputLimit, float maxOutputLimit) {
		this.name = name;
	...
	public float getTotal(float error) {
		float total = (error * 1/p) + (1/i * getIntegralSum()) + (d * getDerivative());
		if (total > maximumLimit) {
			return maximumLimit;
		} else if (total < minimumLimit) {
			return minimumLimit;
		}
		return total;
	}
