...
public class LowPassFilter {
	...
	public void set_cutoff_frequency(float sample_freq, float cutoff_freq) {
		computeParams(sample_freq, cutoff_freq);
	}
	...
	private void computeParams(float sample_freq, float cutoff_freq) {
		this.cutoff_freq = cutoff_freq;
		this.sample_freq = sample_freq;

		Float fr = sample_freq / cutoff_freq;
		Double ohm = Math.tan(Math.PI / fr);
		float c = (float) (1.0f + 2.0f * Math.cos(Math.PI / 4.0f) * ohm + ohm * ohm);

		this.b0 = ohm * ohm / c;
		this.b1 = (float) (2.0f * b0);
		this.b2 = (float) b0;
